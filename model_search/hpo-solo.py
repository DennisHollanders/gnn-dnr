...
WORKER_DATALOADERS = {}
class HPO:
    """Sequential HPO on GPU/CPU with multi-threaded data loading."""

    def __init__(self,
                 config_path: str,
                 study_name: str,
                 startup_trials: int = 25,
                 pruner_startup: int = 15,
                 pruner_warmup: int = 15,
        ...
        self.search_space = cfg.pop('search_space', {})
        self.fixed_params = cfg.pop('fixed_params', {})
        self.config = cfg
    ...
    def _init_csv(self):
        ...
        headers += sorted(self.search_space.keys())
        headers += ['best_mcc', 'final_val_loss', 'final_epoch', 'model_parameters', 'elapsed_time']
    ...
    def run_optimization(self, n_trials: int = 100, use_wandb: bool = False):
        if use_wandb:
            self.wandb_run = wandb.init(
                ...
                name=f"{self.study_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                config={**self.config}
            )
        ...
        if use_wandb:
            self.wandb_run.log({
                "final_best_mcc": study.best_value,
...
def main():
    ...
    hpo = HPO(config_path, args.study_name,trial_timeout=args.trial_timeout)
    hpo.run_optimization(args.trials, args.wandb)
